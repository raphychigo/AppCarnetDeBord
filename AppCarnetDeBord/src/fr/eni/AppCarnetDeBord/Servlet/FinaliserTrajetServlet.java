...
/**
 * Servlet implementation class FinaliserTrajet
 */
@WebServlet("/Login/Conducteur/FinaliserTrajet")
public class FinaliserTrajetServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public FinaliserTrajetServlet() {
        super();
...
HttpSession session = request.getSession();
		...
		RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("finaliserTrajet.jsp");
		if(dispatcher!=null){
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String dateFin = request.getParameter("dateDebutTrajet");
			Date dFin = sdf.parse(dateFin);
			GregorianCalendar gcFin = new GregorianCalendar();
			gcFin.setTime(dFin);			
			trajet.setDateFin(gcFin);
			
			trajet.setKmParcourus(Integer.parseInt(request.getParameter("kmParcourus")));
			trajet.setNbLitres(Double.parseDouble(request.getParameter("nbLitres")));
			trajet.setPrixTotal(Double.parseDouble(request.getParameter("prixTotal")));
			
			DAOTrajet.update(trajet);
		...
		request.getRequestDispatcher("/CarnetDeBord/Administrateur/Trajet").forward(request, response);
	}
